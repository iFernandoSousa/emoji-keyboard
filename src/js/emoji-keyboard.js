...
emojiKeyboard.tabs = [];
emojiKeyboard.tabContents = [];

//TODO: Think about loading this dynamically since it could be very large?
emojiKeyboard.categories = ["emoticons", "nature", "objects", "places", "other"];
emojiKeyboard.emoji = {
...
emojiKeyboard.show = function () {
    var cat, catId, id, unicode,
        tabs = "<ul id='tabs'>",
        content = "<div class='tab-contents'>";

    content += "";

    for (catId in emojiKeyboard.categories) {
        cat = emojiKeyboard.categories[catId];
        tabs += "<li><a href='#" + cat + "'>" + cat + "</a></li>";
        content += "<div id='" + cat + "' class='tab-content'>";

        for (id in emojiKeyboard.emoji[cat])
            content += "<i class='emoji e" + emojiKeyboard.emoji[cat][id] + "'></i>";
    ...
    for (var i = 0; i < tabListItems.length; i++) {
        if (tabListItems[i].nodeName == "LI") {
            var tabLink = emojiKeyboard.getFirstChildWithTagName(tabListItems[i], 'A');
            var id = emojiKeyboard.getHash(tabLink.getAttribute('href'));
            emojiKeyboard.tabs[id] = tabLink;
            emojiKeyboard.tabContents[id] = document.getElementById(id);
        }
    }
    ...
    for (var id in emojiKeyboard.tabs) {
        emojiKeyboard.tabs[id].parentElement.onclick = emojiKeyboard.showTab;
        emojiKeyboard.tabs[id].parentElement = function () {
            this.blur()
        };
        if (i == 0) emojiKeyboard.tabs[id].parentElement.className = 'selected';
        i++;
    }
    ...
    for (var id in emojiKeyboard.tabContents) {
        if (i != 0) emojiKeyboard.tabContents[id].className = 'tab-content hide';
        i++;
        i++;
    }
}
...
emojiKeyboard.showTab = function () {
    var selectedId = emojiKeyboard.getHash(this.getElementsByTagName('a')[0].getAttribute('href'));

    // Highlight the selected tab, and dim all others.
    // Also show the selected content div, and hide all others.
    for (var id in emojiKeyboard.tabContents) {
        if (id == selectedId) {
            emojiKeyboard.tabs[id].parentElement.className = 'selected';
            emojiKeyboard.tabContents[id].className = 'tab-content';
        } else {
            emojiKeyboard.tabs[id].parentElement.className = '';
            emojiKeyboard.tabContents[id].className = 'tab-content hide';
        }
    }
...
emojiKeyboard.getFirstChildWithTagName = function (element, tagName) {
    for (var i = 0; i < element.childNodes.length; i++) {
        if (element.childNodes[i].nodeName == tagName) return element.childNodes[i];
    }
}

emojiKeyboard.getHash = function (url) {
