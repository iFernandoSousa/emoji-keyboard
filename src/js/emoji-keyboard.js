...
var emojiKeyboard = function (options) {
    this.options = options;
};

//using this to keep track of the tabs for the keyboard
emojiKeyboard.prototype.tabs = [];
emojiKeyboard.prototype.tabContents = [];

//The real data of the emoji to display this keyboard
//TODO: Think about loading this dynamically since it could be very large?
emojiKeyboard.prototype.categories = ["emoticons", "nature", "objects", "places", "other"];
emojiKeyboard.prototype.emoji = {
...
//main function of the keyboard, show it tho the world..
emojiKeyboard.prototype.show = function () {
    ...
    //build the tabs and add emoji to the right tab.
    ...
    for (var i = 0; i < tabListItems.length; i++) {
        if (tabListItems[i].nodeName == "LI") {
            ...
            this.tabs[id] = tabLink;
            this.tabContents[id] = document.getElementById(id);
        }
    }
    ...
    for (var id in this.tabs) {
        ...
        if (i == 0) this.tabs[id].parentElement.className = 'selected';
        i++;
    }
...
emojiKeyboard.prototype.showTab = function (elem) {
    console.log(this);
    var selectedId = this.getHash(elem.getElementsByTagName('a')[0].getAttribute('href'));

    // Highlight the selected tab, and dim all others.
    // Also show the selected content div, and hide all others.
    for (var id in this.tabContents) {
        if (id == selectedId) {
            this.tabs[id].parentElement.className = 'selected';
            this.tabContents[id].className = 'tab-content';
        } else {
            this.tabs[id].parentElement.className = '';
            this.tabContents[id].className = 'tab-content hide';
        }
    }
...
emojiKeyboard.prototype.getFirstChildWithTagName = function (element, tagName) {
